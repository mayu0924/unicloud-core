...
public class CheckHelper extends TextViewHelper {
    ...
    public CheckHelper(Context context, CompoundButton view, AttributeSet attrs) {
    ...
    private void initAttributeSet(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.XRadioButton);
        //text
        mTextColorChecked = a.getColor(R.styleable.XRadioButton_text_color_checked, 0);

        //icon
        //Vector兼容处理
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            mIconChecked = a.getDrawable(R.styleable.XRadioButton_icon_src_checked);
        } else {
            int checkedId = a.getResourceId(R.styleable.XRadioButton_icon_src_checked, -1);
            if (checkedId != -1)
    ...
    @Override
    public CheckHelper setTextColorNormal(@ColorInt int textColor) {
        if (!mHasCheckedTextColor) {
    ...
    public CheckHelper setTextColorChecked(@ColorInt int textColor) {
    ...
    public CheckHelper setTextColor(@ColorInt int normal, @ColorInt int pressed, @ColorInt int unable, @ColorInt int checked, @ColorInt int selected) {
    ...
    public CheckHelper setIconChecked(Drawable icon) {

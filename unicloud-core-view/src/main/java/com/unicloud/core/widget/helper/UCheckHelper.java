...
public class UCheckHelper extends UTextViewHelper {
    ...
    public UCheckHelper(Context context, CompoundButton view, AttributeSet attrs) {
    ...
    private void initAttributeSet(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.URadioButton);
        //text
        mTextColorChecked = a.getColor(R.styleable.URadioButton_text_color_checked, 0);

        //icon
        //Vector兼容处理
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            mIconChecked = a.getDrawable(R.styleable.URadioButton_icon_src_checked);
        } else {
            int checkedId = a.getResourceId(R.styleable.URadioButton_icon_src_checked, -1);
            if (checkedId != -1)
    ...
    @Override
    public UCheckHelper setTextColorNormal(@ColorInt int textColor) {
        if (!mHasCheckedTextColor) {
    ...
    public UCheckHelper setTextColorChecked(@ColorInt int textColor) {
    ...
    public UCheckHelper setTextColor(@ColorInt int normal, @ColorInt int pressed, @ColorInt int unable, @ColorInt int checked, @ColorInt int selected) {
    ...
    public UCheckHelper setIconChecked(Drawable icon) {

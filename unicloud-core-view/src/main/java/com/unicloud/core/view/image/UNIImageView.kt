package com.unicloud.core.view.image
...
/**
 * CombineBitmap:生成类似钉钉、微信 群聊组合头像Bitmap
 * implementation 'com.github.Othershe:CombineBitmap:1.0.5'
 * @see https://github.com/SheHuan/CombineBitmap
 */
class UNIImageView constructor(
    context: Context,
    attrs: AttributeSet? = null
) : AppCompatImageView(context, attrs) {

    private var isCircle = false // 是否显示为圆形，如果为圆形则设置的corner无效 = false
    private var isCoverSrc = false// border、inner_border是否覆盖图片 = false
    private var borderWidth = 0// 边框宽度 = 0
    private var borderColor = Color.WHITE // 边框颜色
    private var innerBorderWidth = 0// 内层边框宽度 = 0
    private var innerBorderColor = Color.WHITE // 内层边框充色
    private var cornerRadius = 0 // 统一设置圆角半径，优先级高于单独设置每个角的半径 = 0
    private var cornerTopLeftRadius = 0 // 左上角圆角半径 = 0
    private var cornerTopRightRadius = 0 // 右上角圆角半径 = 0
    private var cornerBottomLeftRadius = 0 // 左下角圆角半径 = 0
    private var cornerBottomRightRadius = 0 // 右下角圆角半径 = 0
    private var maskColor = 0 // 遮罩颜色 = 0
    ...
    init {
        val ta = context.obtainStyledAttributes(attrs, R.styleable.UNIImageView, 0, 0)
        for (i in 0 until ta.indexCount) {
            val attr = ta.getIndex(i)
            if (attr == R.styleable.UNIImageView_uni_image_is_cover_src) {
                isCoverSrc = ta.getBoolean(attr, isCoverSrc)
            } else if (attr == R.styleable.UNIImageView_uni_image_is_circle) {
                isCircle = ta.getBoolean(attr, isCircle)
            } else if (attr == R.styleable.UNIImageView_uni_image_border_width) {
                borderWidth = ta.getDimensionPixelSize(attr, borderWidth)
            } else if (attr == R.styleable.UNIImageView_uni_image_border_color) {
                borderColor = ta.getColor(attr, borderColor)
            } else if (attr == R.styleable.UNIImageView_uni_image_inner_border_width) {
                innerBorderWidth = ta.getDimensionPixelSize(attr, innerBorderWidth)
            } else if (attr == R.styleable.UNIImageView_uni_image_inner_border_color) {
                innerBorderColor = ta.getColor(attr, innerBorderColor)
            } else if (attr == R.styleable.UNIImageView_uni_image_corner_radius) {
                cornerRadius = ta.getDimensionPixelSize(attr, cornerRadius)
            } else if (attr == R.styleable.UNIImageView_uni_image_corner_top_left_radius) {
                cornerTopLeftRadius = ta.getDimensionPixelSize(attr, cornerTopLeftRadius)
            } else if (attr == R.styleable.UNIImageView_uni_image_corner_top_right_radius) {
                cornerTopRightRadius = ta.getDimensionPixelSize(attr, cornerTopRightRadius)
            } else if (attr == R.styleable.UNIImageView_uni_image_corner_bottom_left_radius) {
                cornerBottomLeftRadius = ta.getDimensionPixelSize(attr, cornerBottomLeftRadius)
            } else if (attr == R.styleable.UNIImageView_uni_image_corner_bottom_right_radius) {
                cornerBottomRightRadius = ta.getDimensionPixelSize(attr, cornerBottomRightRadius)
            } else if (attr == R.styleable.UNIImageView_uni_image_mask_color) {
    ...
    override fun onDraw(canvas: Canvas) { // 使用图形混合模式来显示指定区域的图片
        canvas.saveLayer(srcRectF, null, Canvas.ALL_SAVE_FLAG)
        if (!isCoverSrc) {
            val sx =
                1.0f * (mWidth - 2 * borderWidth - 2 * innerBorderWidth) / mWidth
            val sy =
                1.0f * (mHeight - 2 * borderWidth - 2 * innerBorderWidth) / mHeight
        ...
        path.reset()
        if (isCircle) {
            path.addCircle(mWidth / 2.0f, mHeight / 2.0f, radius, Path.Direction.CCW)
        } else {
    ...
    private fun initSrcRectF() {
        if (isCircle) {
            radius = Math.min(mWidth, mHeight) / 2.0f

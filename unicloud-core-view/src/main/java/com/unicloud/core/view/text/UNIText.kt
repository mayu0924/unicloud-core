...
class UNIText : AppCompatTextView {
    ...
    private fun initialize(context: Context, attrs: AttributeSet) {
        val arr = context.obtainStyledAttributes(attrs, R.styleable.UNIText)
        mUseOriginal = arr.getBoolean(R.styleable.UNIText_uni_text_original, false)
        mIsHole = arr.getBoolean(R.styleable.UNIText_uni_text_isHole, false)
        mStrokeWidth = arr.getDimension(R.styleable.UNIText_uni_text_strokeWidth, 1f)
        mRadius = arr.getDimension(R.styleable.UNIText_uni_text_corner_radius, -1f)
        ...
        mStartColor =
            arr.getColor(R.styleable.UNIText_uni_text_start_color, Color.parseColor("#00C7FF"))
        mEndColor =
            arr.getColor(R.styleable.UNIText_uni_text_end_color, Color.parseColor("#2295FF"))
        mGradientOrientation = arr.getInteger(R.styleable.UNIText_uni_text_gradient_orientation, 0)
        arr.recycle()
    }
    ...
    private fun refreshBackGroundDrawable() {
        if (!mUseOriginal) {
            ...
            // 是否开启点击动效
            background = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {// 5.0以上水波纹效果
                RippleDrawable(

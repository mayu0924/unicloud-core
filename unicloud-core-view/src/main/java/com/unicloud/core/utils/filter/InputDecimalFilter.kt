 ...
 * 限制数字小数点位数
 */
class InputDecimalFilter : DigitsKeyListener(false, true) {
    private var digits = 2
    fun setDigits(d: Int): InputDecimalFilter {
        digits = d
        return this
    }
...
// and deleting can't break anything
        ...
        for (i in 0 until dstart) {
            if (dest[i] == '.') { // being here means, that a number has
...
// check if the amount of digits is right
        ...
        for (i in start until end) {
            if (source[i] == '.') { // being here means, dot has been inserted
// check if the amount of digits is right
...
// nothing can break
    ...
    companion object {
        private const val TAG = "InputDecimalFilter"
    }
}